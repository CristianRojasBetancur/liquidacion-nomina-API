  ...
  def settle_payroll
    ...
    if @current_user.company.workers == []
      ...
      @current_user.company.workers.each do |worker|
        ...
        other_salary_incomes = 0
        other_salary_incomes = worker.modification.other_salary_incomes if worker.modification != nil# Por ahora cero luego sera segun las modficaciones del empleado
        salary = base_salary * 30 / PERIOD_DAYS
        total_base_SS_parafiscal = salary + other_salary_incomes
        total_base_social_benefits = 0

        if base_salary / MINIMUM_SALARY <= 2 && worker.transport_subsidy.eql?("yes")
        ...
        no_salary_incomes = 0
        no_salary_incomes = worker.modification.no_salary_incomes if worker.modification != nil # Por ahora cero luego será según las modificaciones del empleado
        total_incomes = total_base_social_benefits + no_salary_incomes
        employee_payment = total_incomes - withholdings_and_deductions(total_base_SS_parafiscal, worker)
        ...
        Payroll.create!({
          ...
          reten_deduc: withholdings_and_deductions(total_base_SS_parafiscal, worker),
      ...
      render :settled, status: :ok
    end
  end

  def withholdings_and_deductions(total_base_SS_parafiscal, worker)
    ...
    other_deductions = 0
    other_deductions = worker.modification.deductions if worker.modification != nil
    deductions = 0
    withholdings_and_deductions = health + pension + solidarity_fund + subsistence_account + other_deductions
