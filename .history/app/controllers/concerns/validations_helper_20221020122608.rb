module ValidationsHelper
  ...
  def valid_modification?
    matched_modification = @current_user.company.periods.modifications.find_by(worker_id: params[:modification][:worker_id])

    unless matched_modification.nil?
      render json: {error: {
        code: "036",
        message: "Already exists payroll modification for employee with id #{params[:modification][:worker_id]}",
        object: "Modification"
      }}
    end
  end
